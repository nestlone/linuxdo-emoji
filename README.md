...
当设备只有 4 核或更少的 CPU、或可用设备内存为 4 GB 或更低时，脚本会自动启用低性能模式：

- 每页仅显示 12 个表情，避免一屏同时解码大量图片。
- GIF、WebP 与 AVIF 在网格中显示为 96px 静态首帧；悬浮预览仍会加载原动图。
- 图片下载并发降为 2，且不执行后台预热。

可在用户脚本菜单的“🐢 设置低性能模式”中选择自动检测、始终开启或始终关闭；重新打开表情选择器后生效。

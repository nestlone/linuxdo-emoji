...
3. **🎞️ 限流分片渲染（Progressive Chunk Rendering）**：首屏仅挂载 18 个表情，后续通过 `requestAnimationFrame` 追加，避免集中创建和解码图片造成卡顿。
4. **🚀 空闲预热与单次下载**：弹窗打开且浏览器空闲后才预热图片；同一图片的显示与离线缓存复用一份 Blob，避免重复网络请求与逐张淡入闪烁。
5. **🧹 受控离线图片缓存**：默认最多缓存 500 张、100 MB；每 7 天清理从未实际使用的图片，超过上限时优先清理使用次数最少的缓存。可在油猴菜单的「💾 设置离线图片缓存上限」中修改。
